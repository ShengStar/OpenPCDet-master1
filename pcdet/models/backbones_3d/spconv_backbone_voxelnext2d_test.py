...
class VoxelResBackBone8xVoxelNeXt2D_TEST(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        self.voxel2pillar = spconv.SparseSequential(
            spconv.SparseConv3d(33, 33, [21,1,1], stride=1, padding=0,bias=False, indice_key='res0'),
            norm_fn(33),
            nn.ReLU()
        )
        ...
        self.conv1 = spconv.SparseSequential(
            SparseBasicBlock(33, 33, norm_fn=norm_fn, indice_key='res1'),
            SparseBasicBlock(33, 33, norm_fn=norm_fn, indice_key='res1'),
            SparseBasicBlock(33, 33, norm_fn=norm_fn, indice_key='res1'),
        )
        ...
        self.conv2 = spconv.SparseSequential(
            # [1600, 1408] <- [800, 704]
            block(33, 64, spconv_kernel_sizes[0], norm_fn=norm_fn, stride=2, padding=int(spconv_kernel_sizes[0]//2), indice_key='spconv2', conv_type='spconv'),
    ...
    def forward(self, batch_dict):
        pillar_features, pillar_coords = batch_dict['pillar_features'], batch_dict['voxel_coords']
        ...
        input_sp_tensor = self.voxel2pillar(input_sp_tensor)
        # print("?????????")
        # print(input_sp_tensor.indices[:,[0,2,3]])
        # print(input_sp_tensor.indices.shape)
        # print(input_sp_tensor.features.shape)
        # print(input_sp_tensor.spatial_shape)
        input_sp_tensor = spconv.SparseConvTensor(
            features=input_sp_tensor.features,
            indices=input_sp_tensor.indices[:,[0,2,3]],
            spatial_shape=[1504, 1504],
            batch_size=input_sp_tensor.batch_size
        )
